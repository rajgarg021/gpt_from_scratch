...
# helps to have multiple channels of communication as the tokens have a lot to talk about (different kinds of information can be aggregated)
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.proj = nn.Linear(num_embeddings, num_embeddings) # projection layer to feed back into the residual pathway  
...
# adding computation to the network (per node level) so that the tokens get time to think on what they found via communication using multi head self-attention
class FeedForward(nn.Module):
    """ two linear layers with a non-linearlity in between """

    def __init__(self, num_embeddings):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(num_embeddings, 4 * num_embeddings), # inner layer dimensionality is 4 * input layer dimensionality in the transformer paper
            nn.ReLU(),
            nn.Linear(4 * num_embeddings, num_embeddings), # acts as the projections layer to feed back into the residual pathway
        )
    
    def forward(self, x):
        return self.net(x)
    
# grouping communication and computaiton to form the transformer block
class Block(nn.Module):
    ...
    def forward(self, x):
        # added residual connections and layer normalization (pre-LN formulation)
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential( # interspersing communcation and computation by replicating the transformer block sequentially
